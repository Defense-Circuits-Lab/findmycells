...
class MicroscopyImageReaders(DataReader):
    ...
    def _get_adapted_configs_value(self,
                                   ...
                                   value_to_be_replaced: Optional[Any]=None,
                                   value_to_use_instead: Optional[Any]=None
                                  ) -> Any:
        configs_value = getattr(self, configs_attribute_key)
        if replace_value:
...
class CZIReader(MicroscopyImageReaders):
    ...
    def read(self,
        ...
        color_channel_idx = self._get_adapted_configs_value(configs_attribute_key = 'color_channel_idx',
                                                            replace_value = True,
                                                            value_to_be_replaced = None,
                                                            value_to_use_instead = slice(None))
        plane_idx = self._get_adapted_configs_value(configs_attribute_key = 'plane_idx',
                                                    replace_value = True,
                                                    value_to_be_replaced = None,
                                                    value_to_use_instead = slice(None))
        version_idx = self._get_adapted_configs_value(configs_attribute_key = 'version_idx',
                                                      replace_value = True,
                                                      value_to_be_replaced = None,
                                                      value_to_use_instead = 0)
        title_row_idx, title_col_idx = self._get_adapted_configs_value(configs_attribute_key = 'title_row_col_idxs',
                                                                       replace_value = True,
                                                                       value_to_be_replaced = None,
                                                                       value_to_use_instead = (0, 0))      
...
class FromExcelReader(MicroscopyImageReaders):
    ...
    def read(self,
        ...
        for row_index in range(df_single_plane_filepaths.shape[0]):
            ...
            file_extension = single_plane_image_filepath.name[single_plane_image_filepath.name.find('.'):]
