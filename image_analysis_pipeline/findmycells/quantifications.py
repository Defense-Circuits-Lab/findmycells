...
class QuantificationObject:
    ...
    def save_postprocessed_segmentations_on_disk(self) -> None:
        for plane_index in range(self.postprocessed_segmentations.shape[0]):
            image = self.postprocessed_segmentations[plane_index].astype('uint8')
            filepath_out = self.database.quantified_segmentations_dir.joinpath(f'{self.file_id}-{str(plane_index).zfill(3)}_quantified_segmentation.png')
            imsave(filepath_out, image, check_contrast=False)   

            
    def update_database(self) -> None:
        self.database.update_file_infos(file_id = self.file_id, updates = {'quantifications_completed': True})
...
class CountInstanceLabelsInWholeAreaROIs(QuantificationStrategy):
    ...
    def get_instance_label_info(self, quantification_object: QuantificationObject) -> Dict:
        ...
        for label_id in instance_label_ids:
            ...
            planes_with_label_id = list()
            for plane_index in range(quantification_object.postprocessed_segmentations.shape[0]):
    ...
    def update_database(self, database: Database, file_id: str, step: int) -> Database:
        updates = dict()
        updates[f'quantification_step_{str(step).zfill(2)}'] = 'CountInstanceLabelsInWholeAreaROIs'
        # Add additional information if neccessary
        database.update_file_infos(file_id = file_id, updates = updates)
        return database           
