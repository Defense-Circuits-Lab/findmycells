...
class QuantificationObject:
    ...
    def __init__(self, database: Database, file_id: str) -> None:
        ...
        if self.database.quantification_configs['segmentations_to_use'] == 'semantic':
    ...
    def run_all_postprocessing_steps(self) -> None: 
        if 'postprocessing_strategies' in self.database.quantification_configs.keys():
            for postprocessing_strategy in self.database.quantification_configs['postprocessing_strategies']:
                self = postprocessing_strategy().run(quantification_object = self, 
                                                       step = self.database.quantification_configs['postprocessing_strategies'].index(postprocessing_strategy))
            
    
    def save_segmentations_used_for_quantifications(self) -> None:
        for area_roi_id in self.segmentations_per_area_roi_id.keys():
            for plane_index in range(self.segmentations_per_area_roi_id[area_roi_id].shape[0]):
                ...
                if filepath.is_dir() == False:
                    filepath.mkdir()
                filename_path = filepath.joinpath(f'{self.file_id}-{str(plane_index).zfill(3)}_quantified_segmentation.png')
                imsave(filename_path, image, check_contrast=False) 

    
    def run_all_quantification_steps(self) -> None:
        if 'quantification_strategies' in self.database.quantification_configs.keys():
            for quantification_strategy in self.database.quantification_configs['quantification_strategies']:
                self = quantification_strategy().run(quantification_object = self, 
                                                       step = self.database.quantification_configs['quantification_strategies'].index(quantification_strategy))          

            
    def update_database(self) -> None:
        updates = {'postprocessing_completed': True,
...
class PostprocessingStrategy(ABC):
    ...
    @abstractmethod
    def run(self, quantification_object: QuantificationObject, step: int) -> QuantificationObject:
        # do postprocessing of segmentations before quantifations can be performed
        quantification_object.database = self.update_database(database = quantification_object.database, file_id = quantification_object.file_id, step = step)
        return quantification_object
    
    @abstractmethod
    def update_database(self, database: Database, file_id: str, step: int) -> Database:
        updates = dict()
        updates[f'postprocessing_step_{str(step).zfill(2)}'] = 'PostprocessingStrategyName'
...
class ReconstructCellsIn3DFrom2DInstanceLabels(PostprocessingStrategy):
    ...
    def run(self, quantification_object: QuantificationObject, step: int) -> QuantificationObject:
        ...
        quantification_object.postprocessed_segmentations, roi_matching_results = self.run_3d_instance_reconstruction(zstack = quantification_object.postprocessed_segmentations)
        quantification_object.database = self.save_multimatches_traceback_to_database(database = quantification_object.database,
                                                                                      file_id = quantification_object.file_id,
    ...
    def update_database(self, database: Database, file_id: str, step: int) -> Database:
        updates = dict()
        updates[f'postprocessing_step_{str(step).zfill(2)}'] = 'ReconstructCellsIn3DFrom2DInstanceLabels'
        database.update_file_infos(file_id = file_id, updates = updates)
        return database           
...
class ApplyExclusionCriteria(PostprocessingStrategy):
    ...
    def run(self, quantification_object: QuantificationObject, step: int) -> QuantificationObject:
        ...
        instance_label_info = self.get_instance_label_info(quantification_object = quantification_object)
        self.exclusion_criteria = self.get_exclusion_criteria(quantification_object = quantification_object)
        all_area_roi_ids = self.get_all_unique_area_roi_ids(rois_dict = quantification_object.rois_dict)
        segmentations_per_area_roi_id = dict()
        for area_roi_id in all_area_roi_ids:
            segmentations_per_area_roi_id[area_roi_id] = self.apply_exclusion_criteria(zstack_prior_to_exclusion = quantification_object.postprocessed_segmentations,
                                                                                       area_roi_id = area_roi_id,
                                                                                       info = instance_label_info)
        quantification_object.segmentations_per_area_roi_id = segmentations_per_area_roi_id
        quantification_object.database = self.update_database(database = quantification_object.database, file_id = quantification_object.file_id, step = step)
        return quantification_object

    
    def get_instance_label_info(self, quantification_object: QuantificationObject) -> Dict:
        instance_label_ids = list(np.unique(quantification_object.postprocessed_segmentations))
        ...
        for label_id in instance_label_ids:
            ...
            plane_indices_with_label_id = list(set(np.where(quantification_object.postprocessed_segmentations == label_id)[0]))
        ...
        instance_label_info = self.extend_info_with_relative_positions(info = instance_label_info, 
                                                                       rois_dict = quantification_object.rois_dict,
                                                                       zstack = quantification_object.postprocessed_segmentations)
    ...
    def get_exclusion_criteria(self, quantification_object: QuantificationObject) -> Dict:
        database = quantification_object.database
        total_planes = quantification_object.postprocessed_segmentations.shape[0]
    ...
    def update_database(self, database: Database, file_id: str, step: int) -> Database:
        updates = dict()
        updates[f'postprocessing_step_{str(step).zfill(2)}'] = 'ApplyExclusionCriteria'
...
class QuantificationStrategy(ABC):
    ...
    @abstractmethod
    def run(self, quantification_object: QuantificationObject, step: int) -> QuantificationObject:
        # Quantify (postprocessed) segmentations
        quantification_object.database = self.update_database(database = quantification_object.database, file_id = quantification_object.file_id, step = step)
        return quantification_object
    ...
    def update_database(self, database: Database, file_id: str, step: int) -> Database:
        updates = dict()
        updates[f'quantification_step_{str(step).zfill(2)}'] = 'QuantificationStrategyName'
        # Add additional information if neccessary
        database.update_file_infos(file_id = file_id, updates = updates)
        return database   
...
class CountFeaturesInWholeAreaROIs(QuantificationStrategy):
    ...
    def run(self, quantification_object: QuantificationObject, step: int) -> QuantificationObject:
        ...
        self.quantification_results = dict()
        for area_roi_id in quantification_object.segmentations_per_area_roi_id.keys():
            ...
            self.quantification_results[area_roi_id] = feature_count
    ...
    def update_database(self, database: Database, file_id: str, step: int) -> Database:
        database.update_file_infos(file_id = file_id, updates = {f'quantification_step_{str(step).zfill(2)}': 'CountFeaturesInWholeAreaROIs'})
        if hasattr(database, 'quantification_results') == False:
            setattr(database, 'quantification_results', dict())
        if 'CountFeaturesInWholeAreaROIs' not in database.quantification_results.keys():
            database.quantification_results['CountFeaturesInWholeAreaROIs'] = dict()
        database.quantification_results['CountFeaturesInWholeAreaROIs'][file_id] = self.quantification_results     
        return database