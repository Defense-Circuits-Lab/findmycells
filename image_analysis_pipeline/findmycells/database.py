...
class Database():
    ...
    def create_deepflash2_subdirectories(self):      
        try: self.trained_models_dir = self.deepflash2_dir + [elem for elem in os.listdir(self.deepflash2_dir) if 'models' in elem][0] + '/'
        except:
            self.trained_models_dir = self.deepflash2_dir + 'trained_models/'
            os.mkdir(self.trained_models_dir)
            
        try: self.deepflash2_temp_dir = self.deepflash2_dir + [elem for elem in os.listdir(self.deepflash2_dir) if 'temp' in elem][0] + '/'
        except:
    ...
    def create_file_infos(self):
        ...
        self.file_infos = {'file_id': list(),
                           ...
                           'imaging_planes': list(),
                           'RGB': list(),
                           'cropping_mode': list(), # e.g. None, StitchingArtefacts, Boundary
            ...
            for subject in os.listdir(self.microscopy_image_dir + group + '/'):
                for filename in os.listdir(self.microscopy_image_dir + group + '/' + subject + '/'):
                    ...
                    try:
                        ...
                        raise FileNotFoundError(f'Couldn´t find matching roi file for {original_file_id}.')
